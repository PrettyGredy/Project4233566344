...
namespace SHG.AnimatorCoder
{
    public abstract class AnimatorCoder : MonoBehaviour
    {
        ...
        public bool IsCurrentAnimationFinished(int layer)
        {
            try
            {
                AnimatorStateInfo currentState = animator.GetCurrentAnimatorStateInfo(layer);

                // Проверка, завершилась ли анимация
                return currentState.normalizedTime >= 1 && !animator.IsInTransition(layer);
            }
            catch
            {
                LogError("Ошибка проверки завершения анимации. Убедитесь, что Initialize() вызван, и слой указан корректно.");
                return false;
            }
        }
    }
